...
#pragma HLS INTERFACE s_axilite port=return
    ...
    roi.x = 1;//TODO: change to a random number < cols_in - cols_out
    roi.y = 2;//TODO: change to a random number < rows_in - rows_out
    roi.height = cols_out;
    roi.width = rows_out;

    xf::cv::Mat<TYPE, HEIGHT, WIDTH, NPC_T> imgInput0(rows_in, cols_in);
    xf::cv::Mat<TYPE, NEWHEIGHT, NEWWIDTH, NPC_T> out_mat_crop(rows_out_resize, cols_out_resize);
...
// clang-format on	
    ...
    xf::cv::crop<TYPE, HEIGHT, WIDTH, MEMORYMAPPED_ARCH, NPC>(imgInput0, out_mat_crop, roi);
    // xf::cv::resize<INTERPOLATION,TYPE,HEIGHT,WIDTH,NEWHEIGHT,NEWWIDTH,NPC_T,MAXDOWNSCALE> (imgInput0, out_mat_crop);
    xf::cv::insertBorder<TYPE, NEWHEIGHT, NEWWIDTH, NEWHEIGHT, NEWWIDTH, NPC_T>(out_mat_crop, out_mat, insert_pad_value);
