...
#ifndef _hls_xfft2real_deshbi_H_
#define _hls_xfft2real_deshbi_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "hls_xfft2real_deshbi_memcore.h"

SC_MODULE(hls_xfft2real_deshbi) {
...
protected:
    ...
    hls_xfft2real_deshbi_memcore* hls_xfft2real_deshbi_memcore_U;
...
public:
    ...
    SC_CTOR(hls_xfft2real_deshbi) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        hls_xfft2real_deshbi_memcore_U = new hls_xfft2real_deshbi_memcore("hls_xfft2real_deshbi_memcore_U");
        hls_xfft2real_deshbi_memcore_U->clk      ( clk );
        hls_xfft2real_deshbi_memcore_U->reset    ( reset );
        hls_xfft2real_deshbi_memcore_U->ce0      ( i_ce0 );
        hls_xfft2real_deshbi_memcore_U->we0      ( i_we0 );
        hls_xfft2real_deshbi_memcore_U->address0 ( memcore_iaddr );
        hls_xfft2real_deshbi_memcore_U->d0       ( i_d0 );
        hls_xfft2real_deshbi_memcore_U->q0       ( i_q0 );

        hls_xfft2real_deshbi_memcore_U->ce1      ( t_ce0 );
        hls_xfft2real_deshbi_memcore_U->we1      ( t_we0 );
        hls_xfft2real_deshbi_memcore_U->address1 ( memcore_taddr );
        hls_xfft2real_deshbi_memcore_U->d1       ( t_d0 );
        hls_xfft2real_deshbi_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "hls_xfft2real_deshbi.trace");
            // ports
