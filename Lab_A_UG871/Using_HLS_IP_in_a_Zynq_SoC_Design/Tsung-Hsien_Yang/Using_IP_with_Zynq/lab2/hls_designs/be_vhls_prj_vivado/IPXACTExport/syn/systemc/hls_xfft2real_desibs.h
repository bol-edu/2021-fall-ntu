...
#ifndef _hls_xfft2real_desibs_H_
#define _hls_xfft2real_desibs_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "hls_xfft2real_desibs_memcore.h"

SC_MODULE(hls_xfft2real_desibs) {
...
protected:
    ...
    // buffer instances
    hls_xfft2real_desibs_memcore* buffer[BufferCount];
...
public:
    ...
    SC_CTOR(hls_xfft2real_desibs) : m_trace_file(0) {
        for (unsigned i = 0; i < BufferCount; i++) {
            char szTmp[128];
            sprintf(szTmp, "%d", i);
            std::string name = "hls_xfft2real_desibs_memcore_U_";
            name += szTmp;

            buffer[i] = new hls_xfft2real_desibs_memcore(name.c_str());
            buffer[i]->clk      ( clk );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "hls_xfft2real_desibs.trace");
            // ports
